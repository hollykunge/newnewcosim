...
public class SysOrgSyncService implements LdapDbSync, SysOrgSyncServiceMBean {
	...
	protected final List<ISysOrg> newFromLdapOrgList = new ArrayList<ISysOrg>();
	protected final List<ISysOrg> deleteLocalOrgList = new ArrayList<ISysOrg>();;
	protected final List<ISysOrg> updateLocalOrgList = new ArrayList<ISysOrg>();;
	...
	protected int syncLdapAprior() {
		 ...
		 */
		// 取得数据库中所有来自AD的组织的路径
		List<ISysOrg> dbAdOrgs = OrgHelper.getSysOrgByFromType(totalDbOrgs, SystemConst.FROMTYPE_AD);
		...
		for (Map<String, Object> map1 : list) {
			...
			for (Map<String, Object> map2 : namePahtsMaps) {
				if (((String) map2.get("path")).equals(path)) {
					...
					ISysOrg org = OrgHelper.getSysOrgById(totalDbOrgs, id);
					map1.put("id", id);
		...
		// 计算组织的父节点。
		List<ISysOrg> addSysOrgs = new ArrayList<ISysOrg>();
		for (Map<String, Object> map1 : list) {
		...
		for (ISysOrg org : addSysOrgs) {
			if (!org.getPath().startsWith("1.")) {
				String path = org.getPath();
				Long ancestorId = Long.parseLong(path.split("\\.")[0]);
				ISysOrg ancestroId = OrgHelper.getSysOrgById(totalDbOrgs, ancestorId);
