...
////原来为什么要这样写？在使用时，发现在添加数据源时，如果别名有大写字母会找不到数据源
		...
		if(!dataSouceMap.containsKey(alias)){
			...
			DataSourceTransactionManager tansactionManager = new DataSourceTransactionManager(source);
			TransactionTemplate tansactionTemplate = new TransactionTemplate(tansactionManager);
	...
	public JdbcTemplate getJdbcTemplate(){
		...
		final JdbcTemplate jdbcTemplate=(JdbcTemplate)map.get("jdbcTemplate");
		return jdbcTemplate;
	}
	...
	public  void execute(final JdbcCommand cmd) {
		...
		Map map= dataSouceMap.get( alias);
		TransactionTemplate tansactionTemplate=(TransactionTemplate)map.get("tansactionTemplate");
		final JdbcTemplate jdbcTemplate=(JdbcTemplate)map.get("jdbcTemplate");
	...
	/**
	 * 查询int返回值。
	 * @param sql select count(*) from sys_role where name=:name
	...
	/**
	 * 查询列表 ，列表中为map对象。
	 * @param sql 示例：select * from sys_role where name=:name
	 * @param parameter Map map=new HashMap(); 		map.put("name", "li");
	 * @return
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	 ...
	 * @param sql		sql语句可以使用?参数化。
	 * @param paramMaps	参数
	 * @param pageBean	是否需要分页。
	 * @return
	 */
	public  List<Map<String,Object>> queryForList(String sql,Map<String,Object> paramMaps,PageBean pageBean)
	{
	...
	public  T queryForObject(T obj,RowMapper rowMap)
	{
		...
		String sql="select a.* from " + objHelper.getTableName() +" a where " + column.getColumnName() +"=:" + column.getPropery();
	 ...
	 * </pre>
	 * @param sql			sql语句。
	 * @param rse			ResultSetExtractor 实现类。
	 * @param pageBean		分页对象。
	 * @param params		参数
	 * @return
	 * @throws Exception
	 */
	@SuppressWarnings("rawtypes")	
	public  T getPage(String sql,ResultSetExtractor<T> rse,PageBean pageBean,Map<String,Object> params) throws Exception
	{
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public  List<T> getPage(int currentPage,int pageSize,String sql,Map paraMap,PageBean pageBean)
	{
