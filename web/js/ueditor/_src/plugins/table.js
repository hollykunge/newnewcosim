...
UE.plugins['table'] = function() {
    ...
    me.addListener('keydown', function(type, evt) {
        ...
        if (!keys[keyCode] && !evt.ctrlKey && !evt.metaKey && !evt.shiftKey && !evt.altKey) {
        	removeClass(me.currentSelectedArr,"selectTdClass");
        }
    });
    ...
    me.commands['tableformat']={
		queryCommandState : function() {
			...
			if (count > 1) {
				...
				var range = this.selection.getRange();
				return domUtils.findParentByTagName(range.startContainer,
						'table', true) ? 0 : (domUtils.findParentByTagName(
						range.endContainer, 'table', true) ? 0 : -1);
			}
		},
		execCommand : function() {
			var range = this.selection.getRange(),
            ...
            table = domUtils.findParentByTagName(td, 'table');
			if(td.className!=""){
				editor.tableFormatState=table.className;
				editor.cellFormatState=td.className;
			}
		}    		
    };
            ...
            for (var rowIndex = 0; rowIndex < rows.length;) {
                ...
                if (_isHide(tmpCell)) {//如果当前单元格是隐藏的，表明当前单元格由其左边span过来，找到其左边单元格
                    ...
                    for (var i = 0; i < leftCell.rowSpan; i++) {
                        ...
                        tr = rows[rowIndex + i];
                        if (tr.children[cellIndex]) {                        	
                        	utils.insertAfter(newCell, tr.children[cellIndex]);
                        } else {
                            tr.appendChild(newCell);
                        }
    ...
    function _toggleSelect(table, cellsRange) {
        var rows = table.rows,selectClass="selectTdClass";        
        removeClass(me.currentSelectedArr,selectClass);
        for (var i = cellsRange.beginRowIndex; i <= cellsRange.endRowIndex; i++) {
            ...
            addClass(me.currentSelectedArr,selectClass);
        }
    }

    //更新rootRowIndxe,rootCellIndex
    function update(table,resize) {
        ...
        for (var j = 0,tj; tj = tds[j++];) {
            ...
            var width;
            if(!resize&&!_isHide(tj) && (width = tj.style.width) && /%/.test(width)){
