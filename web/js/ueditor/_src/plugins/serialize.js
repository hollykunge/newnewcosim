...
UE.plugins['serialize'] = function() {
			...
			while ((match = RE_PART.exec(html))) {
				...
				if ((tagName = match[1])) {
					...
					if (cdata && tagName == cdata._tag_name) {
						callbacks.onCDATA(cdata.join(''),tagIndex);
						cdata = null;
					}
					if (!cdata) {
				...
				if ((tagName = match[3])) {
					...
					tagName = tagName.toLowerCase();
					var attrPart = match[4], attrMatch, attrMap = {}, selfClosing = attrPart
							&& attrPart.slice(-1) == '/';
					if (attrPart) {
						RE_ATTR.exec("");
		...
		return function(html, forceDtd) {
			...
			function addElement(element, open) {
				...
				if (forceDtd) {
					...
					while (dtd[node.tag] && !(currentNode.tag == 'span' ? utils.extend(
									dtd['strong'], {
										'a' : 1,
										'A' : 1
			...
			parse(html,
					{
						...
						onCDATA : function(text,tagIndex) {
							while (!(dtd[currentNode.tag] || dtd['div'])['#']) {
								if (tagEnd(currentNode))
									continue;
								currentNode = currentNode.parent;
	...
	var toHTML = function() {
			...
			for ( var k in attrs) {
				...
				if (k == 'style') {
					...
					if (/rgba?\s*\([^)]*\)/.test(value)) {
						value = value.replace(/rgba?\s*\(([^)]*)\)/g, function(
								str) {
					...
					// 所以在这里去掉这个属性
					attrs[k] = utils.optCss(
							value.replace(/windowtext/g, '#000')).replace(
							/white-space[^;]+;/g, '');
		...
		function printData(node, notTrans) {
			...
			// node.data.replace(/&nbsp;/g,' ')
			// 针对pre中的空格和出现的&nbsp;把他们在得到的html代码中都转换成为空格，为了在源码模式下显示为空格而不是&nbsp;
			return notTrans ? node.data.replace(/&nbsp;/g, ' ') : unhtml1(
					node.data).replace(/ /g, '&nbsp;');
		}
		...
		function printElement(node, pasteplain) {
			if (node.type == 'element' && !node.children.length
					&& (dtd.$removeEmpty[node.tag]) && node.tag != 'a'
					&& utils.isEmptyObject(node.attributes)
					&& autoClearEmptyNode) {// 锚点保留
				return html;
			}
			var tag = node.tag;
			if (pasteplain && tag == 'td') {
				...
				if (!EMPTY_TAG[tag]) {
					...
					html += printChildren(node, pasteplain);
					html += '</'
							+ (pasteplain && transHtml[tag] ? transHtml[tag]
									: tag) + '>';
				}
			}
	...
	var transformWordHtml = function() {
		...
		function filterPasteWord(str) {
			str = str
					...
					// 去掉多余的属性
					.replace(/v:\w+=["']?[^'"]+["']?/g, '')
					// Remove comments, scripts (e.g., msoShowComment), XML tag,
					// VML content, MS Office namespaced tags, and a few other
					// tags
					.replace(
			...
			// Examine all styles: delete junk, transform some, and keep the
			// rest
			...
			str = str
					.replace(
							...
							function(str, tag, tmp, style) {
								...
								var n = [], i = 0, s = style.replace(
										/^\s+|\s+$/, '').replace(/&quot;/gi,
										"'").split(/;\s*/g);

								// Examine each style definition within the
								...
								for ( var i = 0; i < s.length; i++) {
									...
									if (parts.length == 2) {
										...
										// Translate certain MS Office styles
										// into their CSS equivalents
										switch (name) {
								...
								// If style attribute contained any valid styles
								// the re-write it; otherwise delete style
								// attribute.
								if (i > 0) {
	...
	// if ( node && node.tag == 'p' ) {
	// //office 2011下有效
	// if ( node.attributes['class'] == 'MsoListParagraph' || /mso-list/.test(
	// node.attributes.style ) ) {
	// a = 1;
	// } else {
	// var firstChild = node.children[0];
	// if ( firstChild && firstChild.tag == 'span' && /Wingdings/i.test(
	...
	function transNode(node, word_img_flag) {
		...
		case 'textarea':
			if (browser.ie) {
				node.tag = 'input';
				node.attributes._ue_input_textarea = 1;
				node.attributes._ue_textarea_style = node.attributes.style;
				node.attributes.style = 'height:100px;width:320px;';
			}
			break;
		case 'img':
			...
			if (node.attributes.src && /^(?:file)/.test(node.attributes.src)) {
				...
				node.attributes.src = me.options.UEDITOR_HOME_URL
						+ 'themes/default/images/spacer.gif';
				var flag = parseInt(node.attributes.width) < 128
						|| parseInt(node.attributes.height) < 43;
				node.attributes.style = "background:url("
						+ me.options.UEDITOR_HOME_URL
						+ "themes/default/images/"
						+ (flag ? "word.gif" : "localimage.png")
			...
			node.attributes.data_ue_src = node.attributes.data_ue_src
					|| node.attributes.src;
			break;
		case 'li':
		...
		case 'span':
			...
			if (style) {
				...
				style = style
						.match(/(?:\b(?:color|font-size|background(-color)?|font-family|text-decoration)\b\s*:\s*(&[^;]+;|[^;])+(?=;)?)/gi);
				if (style) {
					node.attributes.style = style.join(';');
				}
				if (!node.attributes.style || browser.webkit
			...
			if (browser.gecko && browser.version <= 10902 && node.parent) {
				...
				if (parent.tag == 'span' && parent.attributes
						&& parent.attributes.style) {
					node.attributes.style = parent.attributes.style + ';'
							+ node.attributes.style;
				}
			}
		...
		case 'font':
			...
			node.attributes = {
				'style' : (attr.size ? 'font-size:'
						+ (sizeMap[attr.size] || 12) + 'px' : '')
						+ ';'
			...
			while (node.parent.tag == node.tag
				...
				node.attributes.style
						&& (node.parent.attributes.style ? (node.parent.attributes.style += ";"
								+ node.attributes.style)
	...
	function transOutNode(node) {
		...
		switch (node.tag) {
		case 'input':
			if (node.attributes._ue_input_textarea) {
				node.tag = 'textarea';
				if (node.attributes._ue_textarea_style != "undefined") {
					node.attributes.style = node.attributes._ue_textarea_style;
				} else {
					delete node.attributes.style;
				}
				delete node.attributes._ue_input_textarea;
				delete node.attributes._ue_textarea_style;
			}
			break;
		case 'table':
	...
	Serialize.prototype = {
		...
		filter : function(node, rules, modify) {
			...
			function visitNode(node, parent) {
				node.name = node.type == 'element' ? node.tag
						: NODE_NAME_MAP[node.type];
				if (parent == null) {
				...
				if (whiteList) {
					if (node.type == 'element') {
						if (parent.type == 'fragment' ? whiteList[node.name]
								: whiteList[node.name]
										&& whiteList[parent.name][node.name]) {
