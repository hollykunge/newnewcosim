...
UE.plugins['input'] = function() {
			...
			execCommand : function(cmdName,data,old) {
				if(data){					
					if(old){
						...
						var previewstyle=setStyle(data);
						old.setAttribute("external",getExternal(data));
						old.firstChild.setAttribute("style",previewstyle);
					}
					else{
						var previewstyle='style="'+setStyle(data)+' "  ';
	...
	me.commands['personpicker'] = {
			execCommand : function(cmdName,data,old) {
				if(data){	
					...
					me.curInputHtml=null;
					var html='<tr><th>按钮文本</th><td><input eid="buttoncontent" type="text" value="选择" /></td><th>其他设定:</th><td><label><input eid="singleselect" type="checkbox" />单选</label><label><input eid="isformuser" type="checkbox" />下个节点执行人</label><label><input eid="showCurUser" type="checkbox" />显示当前用户</label></td></tr>';
					me.curInputHtml=getContentHtml(html);
					initDialog(cmdName);
				}
			},
	...
	me.commands['departmentpicker'] = {
			execCommand : function(cmdName,data,old) {				
				if(data){
					...
					me.curInputHtml=null;
					var html='<tr><th>按钮文本</th><td><input eid="buttoncontent" type="text" value="选择" /></td><th>其他设定:</th><td><label><input eid="isformuser" type="checkbox" />部门负责人为下节点执行人</label><label><input eid="showCurOrg" type="checkbox" />显示当前部门</label></td></tr>';
					me.curInputHtml=getContentHtml(html);
					initDialog(cmdName);
				}
			},
	...
	me.commands['subtable'] = {
			execCommand : function(cmdName,data,old) {
				//执行插入html到编辑器的动作
				if(data){
					var num=0,tableName='',tableDesc='',model=0,show='';
					for(var i=0,c;c=data[i++];){
						if(c.id=="tablerows")
							num=c.val;
						if(c.id=='tablename')
				...
				//打开对话框
					...
					html.push('<tr><th>初始化显示</th><td colspan="3" ><label><input eid="show" type="checkbox" /></label></td></tr>');
	...
	function getContentHtml(html){
		var content=['<table class="edit_table"><tr><th>字段描述:</th><td><input eid="comment" type="text" validate="{required:true}"  onblur="getKeyName(this)"/></td><th>字段名称:</th><td><input eid="name" type="text" validate="{required:true,variable:true}"/></td></tr><tr><th>数据类型:</th><td>'];
