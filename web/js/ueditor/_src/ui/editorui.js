...
(function (){
    ...
    var iframeUrlMap ={
        ...
        'input':'~/dialogs/input/input.html',
        'opinion':'~/dialogs/opinion/opinion.html',
        'insertfunction':'~/dialogs/insertfunction/insertfunction.html',
        'importform':'~/dialogs/import/importform.html',
 //       'customdisplay':'~/dialogs/customdisplay/customdisplay.html',  //自定义数据查询显示组件
        'systablemanage':'~/dialogs/systablemanage/systablemanage.html'  //自定义数据查询显示组件
    };
    ...
    var btnCmds = ['undo', 'redo','formatmatch',
        ...
        'tableformat','insertrow','insertcol','insertafterrow','insertaftercol','mergeright','mergedown','deleterow',
        'deletecol','splittorows','splittocols','splittocells','mergecells','deletetable','taskopinion','flowchart','hidedomain','textinput',
        'textarea','checkbox','radioinput','selectinput','dictionary','personpicker','departmentpicker','rolepicker','positionpicker',
        'attachement','datepicker','officecontrol','subtable','ckeditor','exportform','customdisplay','systablemanage'];
    ...
    var dialogBtns = {
        ...
        ok : ['attachment','anchor','link', 'insertimage', 'map', 'gmap','insertframe','wordimage',
            'insertvideo','highlightcode','insertframe','edittd','importform',/*'customdisplay',*/'systablemanage']
    ...
    editorui.input = function (editor,iframeUrl ,title){
    	iframeUrl = iframeUrl || (editor.options.iframeUrlMap||{})['input'] || iframeUrlMap['input'];
    ...
    //统计函数
    editorui.insertfunction=function(editor,iframeUrl ,title){
    	iframeUrl = iframeUrl || (editor.options.iframeUrlMap||{})['insertfunction'] || iframeUrlMap['insertfunction'];
        title = title || editor.options.labelMap['insertfunction'] || '';
        if(!iframeUrl){
            return;
        }
        var dialog = new editorui.Dialog({
            iframeUrl: editor.ui.mapUrl(iframeUrl),
            editor: editor,
            className: 'edui-for-insertfunction',
            title: title,
            buttons: [{
                className: 'edui-okbutton',
                label: '确认',
        ...
        dialog.render();
        editor.ui._dialogs['insertfunctionDialog'] = dialog;
        ...
        var ui = new editorui.Button({
            ...
            title : title || editor.options.labelMap['insertfunction'] || '',
            onclick: function (){               
                editor.execCommand("insertfunction");
            }
        });
        editor.addListener('selectionchange', function (t,v){
            ui.setDisabled(editor.queryCommandState('insertfunction') == -1);
        });
        return ui;
    };
  //添加意见
    editorui.opinion = function (editor,iframeUrl ,title){
    	iframeUrl = iframeUrl || (editor.options.iframeUrlMap||{})['opinion'] || iframeUrlMap['opinion'];
        title = title || editor.options.labelMap['opinion'] || '';
        if(!iframeUrl){
            return;
        }
        var dialog = new editorui.Dialog({
        ...
        dialog.render();
        editor.ui._dialogs['addopinion'] = dialog;    	
        ...
        var ui = new editorui.Button({
            ...
            title : title || editor.options.labelMap['opinion'] || '',
            onclick: function (){               
                editor.execCommand("addopinion");
            }
        });
        editor.addListener('selectionchange', function (t,v){
            ui.setDisabled(editor.queryCommandState('addopinion') == -1);
        });
        return ui;
    };
    ...
    //重新选择模板
    editorui.choosetemplate=function(editor){
    	var ui = new editorui.Button({
            className: 'edui-for-choosetemplate',
            title: editor.options.labelMap.choosetemplate || '',
            onclick: function (){
                editor.execCommand('choosetemplate');
            }
        });    	
        editor.addListener('selectionchange', function (t,v){
            ui.setDisabled(editor.queryCommandState('choosetemplate') == -1);        	
        });
        return ui;
    };
})();
