Namespace.register("com.hotent.platform.desktop");
/**
 * 获取栏目模板
 */
com.hotent.platform.desktop.getColumnTemp= function(cols,width,showTable){
...
com.hotent.platform.desktop.homeTemp= function(cols,width){	
  $.post(
	__ctx+"/platform/system/desktopMycolumn/getMycolumnData.ht",
	function(jsonData){
		var colshtml=com.hotent.platform.desktop.getColumnTemp(cols,width,0);		
 ...
 * 获取栏目数据的模板
 */
com.hotent.platform.desktop.myTemp= function(cols,columnId,width,showTable,desktop){
		    var columnUrl;
	        ...
	        $.ajax({  
	            ...
	            success: function (jsonData) {  
	            	//加载布局模板
	                colshtml=com.hotent.platform.desktop.getColumnTemp(cols,width,showTable,desktop);
	                $("#colshtml").append(colshtml);
 ...
 * 保存布局
 */
com.hotent.platform.desktop.saveCol= function(layoutID,filename,layoutCols){
		var jsonSaveStr = "[";
		// 获取布局数据
		...
		$.ajax({
			...
			success : function(result) {
				...
				if (msg.isSuccess()) {
					ref = true;
					$.ligerMessageBox.success('提示信息', '保存布局成功！');
				} else {
					$.ligerMessageBox.success('提示信息', '保存布局失败！');
					ref = false;
				}
			}
		});
