...
function settingTemplate(){
    ...
    if(!file){
    	$.ligerMessageBox.error("请先选择一个模板！");
    	return;
    }
    
    var settings = getAllSetting();
    $("#fieldSetting").val(JSON.stringify(settings.fieldSetting));
    $("#conditionField").val(JSON.stringify(settings.conditionField));
...
// 	$("#fieldSetting").val(JSON.stringify(rtn.fieldSetting));
	$("#setting").val(JSON.stringify(rtn.setting));
	}
}
...
function constructConditionTr(condition) {
	...
	$hiddenInput.val(JSON.stringify(condition));
