...
function validate(){
	...
	if(valid.status!=0){
		...
		if(valid.message){
			$.ligerMessageBox.error("Error",valid.message);
		}
		return false;
	}
...
function selectTable(){
	...
	if(typeof(dsName)=='undefine' || dsName==""){
		$.ligerMessageBox.warn("提示信息","请选择数据源！");
		return false;
	}
	...
	if (obj.isSuccess()) {
		$.ligerMessageBox.confirm("提示信息", obj.getMessage()+",是否继续操作", function(rtn) {
			if(rtn){
				...
				window.location.href =  _ctx+"/platform/system/sysTableManage/list.ht";
			}
		});
	} else {
...
function constructConditionTr(condition){
	...
	var hiddenInput=constructTag("input",{type:"hidden",value:JSON.stringify(condition)});
...
function constructColumnTr(field){
	...
	hiddenInput.val(JSON.stringify(field));
...
function validateConditionScript(obj){
	...
	var validate=function(vars){
		...
		$.post(url,params,function(data){
			...
			if(data.status){
				$.ligerMessageBox.success("在当前上下文，脚本执行成功","结果:"+data.result);
			}else{
