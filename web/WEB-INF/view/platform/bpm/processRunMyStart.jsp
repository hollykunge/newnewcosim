...
<body>
	<div class="panel panel-default">
		<div class="panel-heading" style="background:#76B0EA">
			<div class="panel-title" style="color:#ffffff;font-weight:bold;font-size:15px;">我启动的流程列表</div>
		</div>
			...
			<div class="panel-search">
				<form id="searchForm" method="post" action="myStart.ht">
					<div class="row">
						...
						<input name="Q_begincreatetime_DL" class="inputText date" value="${param['Q_begincreatetime_DL']}"/>
						<span class="label">至:</span>
						<input name="Q_endcreatetime_DG" class="inputText date" value="${param['Q_endcreatetime_DG']}"/>
						<span class="label">状态:</span>
						<select name="Q_status_SN" value="${param['Q_status_SN']}">
							<option value="">所有</option>
							<option value="1" <c:if test="${param['Q_status_SN'] == 1}">selected</c:if>
							>正在运行
						</option>
						<option value="2" <c:if test="${param['Q_status_SN'] == 2}">selected</c:if>
						>结束
					</option>
				</select>
			</div>
	...
	<display:table name="processRunList" id="processRunItem"
		...
		<display:column title="${checkAll}" media="html" style="width:30px;" class="active">
			<input type="checkbox" class="pk" name="runId"
						value="${processRunItem.runId}"></display:column>
		<display:column property="processName" title="流程定义名称"
					sortable="true" sortName="processName" style="text-align:left" class="active"></display:column>
		<display:column property="subject" title="流程实例标题" sortable="true"
					sortName="subject" style="text-align:left" class="active"></display:column>
		<display:column property="creator" title="创建人" sortable="true"
					sortName="creator" style="text-align:left" class="active"></display:column>
		...
		<display:column title="持续时间" sortable="true" sortName="duration" class="active">${f:getTime(processRunItem.duration)}</display:column>
		...
		<display:column title="管理" media="html"
					style="text-align:left" class="active">
