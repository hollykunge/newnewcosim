...
<div class="panel">
       ...
       <c:choose>
	   		<c:when test="${scope eq 'global' }">
	   			<f:tab curTab="角色授权" tabName="sysOrg"/>
	   		</c:when>
	   		<c:otherwise>
	   			<f:tab curTab="角色授权" tabName="sysOrgGrade"/>
	   		</c:otherwise>
	   </c:choose>
       <c:choose>
					...
					<div class="toolBar">
						...
						<c:choose>
							<c:when test="${scope=='global' }">
								<div class="group"><a class="link del" action="${ctx}/platform/system/sysOrgRole/del.ht">移除</a></div>
							</c:when>
							<c:otherwise>
								<div class="group"><a class="link del" action="${ctx}/platform/system/sysOrgRole/delGrade.ht">移除</a></div>
							</c:otherwise>
						</c:choose>
					</div>	
				</div>
