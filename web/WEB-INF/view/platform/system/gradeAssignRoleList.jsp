...
<div class="panel">
	   <c:choose>
	   		<c:when test="${scope eq 'global' }">
	   			<f:tab curTab="可分配角色" tabName="sysOrg"/>
	   		</c:when>
	   		<c:otherwise>
	   			<f:tab curTab="可分配角色" tabName="sysOrgGrade"/>
	   		</c:otherwise>
	   </c:choose>
					...
					<div class="toolBar">
						...
						<c:choose>
							<c:when test="${scope=='global'}">
								<div class="group"><a class="link del" action="${ctx}/platform/system/grade/del.ht">移除</a></div>
							</c:when>
							<c:otherwise>
								<div class="group"><a class="link del" action="${ctx}/platform/system/grade/delGrade.ht">移除</a></div>
							</c:otherwise>
						</c:choose>
