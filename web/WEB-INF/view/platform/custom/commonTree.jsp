		...
		*/
		function initSetting(setting){
			if(setting){
				...
				$("#nodeId").attr("field",JSON.stringify(node));
				$("#parentNodeId").val(parentNode.name);
				$("#parentNodeId").attr("field",JSON.stringify(parentNode));
				$("#displayField").val(displayField.name);
				$("#displayField").attr("field",JSON.stringify(displayField));
			}
		}
		...
		function constructColumnTr(field){
			...
			$inputHidden.val(JSON.stringify(field));
...
// 			$tr.append($tdSelect);
		...
		function getAllSetting(){
			...
			if(!node){
				$.ligerMessageBox.error("请选择节点ID！");
				return false;
			}
			var parentNode=$("#parentNodeId").attr("field");
			if(!parentNode){
				$.ligerMessageBox.error("请选择父节点ID！");
				return false;
			}
			var display=$("#displayField").attr("field");
			if(!display){
				$.ligerMessageBox.error("请选显示字段！");
				return false;
			}
			var setting = {
				node:$.parseJSON(node),
