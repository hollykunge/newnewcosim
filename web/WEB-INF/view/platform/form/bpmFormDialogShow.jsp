								...
								<form id="searchForm" method="post" action="show.ht">
										<div class="row">
											<c:forEach items="${bpmFormDialog.conditionList}" var="col" >
													<c:if test="${col.defaultType==1 }">
													<span class="label">${col.comment }:</span>
													<c:choose>
														<c:when test="${col.fieldType=='date'}">
															<c:choose>
																<c:when test="${col.condition=='='}">
																	<input type="text" name="Q_${col.fieldName}_DL" class="Wdate inputText"  />
																</c:when>
																<c:otherwise>
																	从:<input type="text" name="Q_start${col.fieldName}_DL" class="Wdate inputText"  />
																	到:<input type="text" name="Q_end${col.fieldName}_DG" class="Wdate inputText"  />
																</c:otherwise>
															</c:choose>
															<input type="text" name="Q_${col.fieldName}"  class="inputText date" />
														</c:when>
