	...
	<div class="panel">
		<div class="panel-top">
			<div class="tbar-title">
				<span class="tbar-label">研发商机管理列表</span>
			</div>
		...
		<div class="panel-body">
		    ...
		    <display:table   name="businessDevchaseList" id="businessDevchaseItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" export="false"  class="table-grid">
				<display:column title="${checkAll}" media="html" style="width:30px;">
			  		<input type="checkbox" class="pk" name="id" value="${businessDevchaseItem.id}">
				</display:column>
				...
				<display:column  title="开始时间" sortable="true" sortName="start_time">
					<fmt:formatDate value="${businessDevchaseItem.startTime}" pattern="yyyy-MM-dd"/>
				</display:column>
				<display:column  title="结束时间" sortable="true" sortName="end_time">
					<fmt:formatDate value="${businessDevchaseItem.endTime}" pattern="yyyy-MM-dd"/>
				</display:column>
				 
				 
				 
				<display:column  title="发布时间" sortable="true" sortName="operate_time">
					<fmt:formatDate value="${businessDevchaseItem.operateTime}" pattern="yyyy-MM-dd"/>
				</display:column>
				...
				<display:column title="管理" media="html" style="width:200px">
					<a href="del.ht?id=${businessDevchaseItem.id}" class="link del">删除</a>
				 
					<a href="get.ht?id=${businessDevchaseItem.id}" class="link detail">明细</a>
					<a href="change.ht?id=${businessDevchaseItem.id}" class="link edit">审核</a>
					
				</display:column>
				 
			</display:table>
			<hotent:paging tableId="businessDevchaseItem"/>
