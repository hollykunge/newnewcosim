	...
	<div class="panel">
		<div class="panel-top">
			<div class="tbar-title">
				<span class="tbar-label">服务商机管理列表</span>
			</div>
		...
		<div class="panel-body">
		    ...
		    <display:table name="businessServechaseList" id="businessServechaseItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" export="false"  class="table-grid">
				<display:column title="${checkAll}" media="html" style="width:30px;">
			  		<input type="checkbox" class="pk" name="id" value="${businessServechaseItem.id}">
				</display:column>
				...
				<display:column  title="开始时间" sortable="true" sortName="start_time">
					<fmt:formatDate value="${businessServechaseItem.startTime}" pattern="yyyy-MM-dd"/>
				</display:column>
				<display:column  title="结束时间" sortable="true" sortName="end_time">
					<fmt:formatDate value="${businessServechaseItem.endTime}" pattern="yyyy-MM-dd"/>
				</display:column>
				 
				 
				 
				<display:column  title="发布时间" sortable="true" sortName="operate_time">
					<fmt:formatDate value="${businessServechaseItem.operateTime}" pattern="yyyy-MM-dd"/>
				</display:column>
				...
				<display:column title="管理" media="html" style="width:200px">
					<a href="del.ht?id=${businessServechaseItem.id}" class="link del">删除</a>
					 
					<a href="get.ht?id=${businessServechaseItem.id}" class="link detail">明细</a>
					<a href="change.ht?id=${businessServechaseItem.id}" class="link edit">审核</a>
					
				</display:column>
			</display:table>
			<hotent:paging tableId="businessServechaseItem"/>
