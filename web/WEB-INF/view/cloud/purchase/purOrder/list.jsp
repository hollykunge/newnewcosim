...
/*
	$("a.update").click(function() {
		$('#purOrderItem').find('input:checked').each(function(){
			alert($(this).val());
				...
				<form id="searchForm" method="post" action="list.ht">
					<div class="row">
						...
						<select name="Q_state_SE" class="inputText" onChange="this.form.submit();">
							...
							<option value="待发货">5-待发货</option>
							<option value="待收货">6-待收货</option>
									<option value="分批入库">7-分批入库</option>
									<option value="全部入库">8-全部入库</option>
									<option value="已结算">9-已结算</option>
									<option value="已支付">10-已支付</option>
						</select>
		...
		<div class="panel-body">
		    ...
		    <display:table name="purOrderList" id="purOrderItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" export="false"  class="table-grid">
				<display:column title="${checkAll}" media="html" style="width:30px;">
			  		...
			  		<input type="hidden" class="pk" name="state" value="${purOrderItem.state}">
				</display:column>
...
<%--				<display:column property="suppenterpUsername" title="供应商联系人姓名" sortable="true" sortName="suppenterp_username"></display:column>
				...
				<c:if test="${purOrderItem.state=='1-下采购订单'}">
					<a href="del.ht?id=${purOrderItem.id}" class="link del">删除</a>
 					<a href="dir_purchase.ht?id=${purOrderItem.id}" class="link edit">编辑</a>
				</c:if>
