...
<script type="text/javascript">
	$(function() {
		$('#ok').click(function() {
				var friend = [];
				$('#saleOrdersList').find(':radio:checked').each(function() {
				...
				window.parent._callBackGetSaleOrders(friend);
			});
	});
...
<div class="panel-top">
			...
			<div class="panel-search">
				<form id="searchForm" method="post" action="getSaleOrders.ht">
	...
	<div class="panel-body">
		<display:table name="saleOrdersList" id="saleOrdersList"
			requestURI="getSaleOrders.ht" sort="external" cellpadding="1" cellspacing="1"
			export="false" class="table-grid">
			<display:column title="${checkAll}" media="html" style="width:30px;">
				<input type="radio" class="pk" name="id" value="${saleOrdersList.id}">
			</display:column>
			...
			<display:column  title="制单日期" sortable="true" sortName="operate_date">
					<fmt:formatDate value="${saleOrdersList.operateDate}" pattern="yyyy-MM-dd"/>
				</display:column>
			<display:column property="state" title="状态" sortable="true" sortName="state"></display:column>
			 	<display:column title="管理" media="html" >
					 
						<c:if test="${saleOrdersList.state=='分批出库'}">
				<a href="getoutDetail.ht?id=${saleOrdersList.id}" class="link detail">明细</a>
	...
	<hotent:paging tableId="saleOrdersList" />
	</div>
