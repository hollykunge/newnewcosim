	...
	<div class="panel">
		<div class="panel-top">
			<div class="tbar-title">
				<span class="tbar-label">销售订单管理列表</span>
			</div>
			...
			<!-- 		<div class="group"><a class="link update" id="btnUpd" action="edit.ht">修改</a></div>
					...
					<div class="l-bar-separator"></div> -->
					<div class="group"><a class="link back"  onClick="location.href='reportOrderByMonth.ht'">月度销售统计图</a></div>
				</div>	
			</div>
			<div class="panel-search">
				<form id="searchForm" method="post" action="list.ht">
					<div class="row">
						<span class="label">销售订单编号:</span><input type="text" name="Q_code_SL"  class="inputText" />
					...
					<!--  <span class="label">制单人姓名:</span><input type="text" name="Q_operatorName_SL"  class="inputText" />
						...
						<span class="label">供应商企业名称:</span><input type="text" name="Q_suppenterpName_SL"  class="inputText" />
		...
		<div class="panel-body">
		    ...
		    <display:table name="saleOrderList" id="saleOrderItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" export="false"  class="table-grid">
				<display:column title="${checkAll}" media="html" style="width:30px;">
			  		<input type="checkbox" class="pk" name="id" value="${saleOrderItem.id}">
				</display:column>
				<display:column property="code" title="销售订单编号" sortable="true" sortName="code"></display:column>
				<display:column  title="制单日期" sortable="true" sortName="operate_date">
					<fmt:formatDate value="${saleOrderItem.operateDate}" pattern="yyyy-MM-dd"/>
				</display:column>
				...
				<display:column property="suppenterpName" title="供应商企业名称" sortable="true" sortName="suppenterp_name"></display:column>
				...
				<display:column property="receiverName" title="收货人姓名" sortable="true" sortName="receiver_name"></display:column>
                                <display:column property="state" title="状态" sortable="true" sortName="state" style="width:80px" ></display:column>
...
<%-- 					<a href="del.ht?id=${saleOrderItem.id}" class="link del">删除</a>
					<a href="edit.ht?id=${saleOrderItem.id}" class="link edit">编辑</a> --%>
					<a href="get.ht?id=${saleOrderItem.id}" class="link detail">明细</a>
					<c:if test="${saleOrderItem.sourceformType == '无源' }">
						<a href="${ctx}/platform/bpm/processRun/userImage.ht?action=process&action=process&runId=${saleOrderItem.runId}" class="link detail">流程示意图</a>					
					</c:if>
				</display:column>
			</display:table>
